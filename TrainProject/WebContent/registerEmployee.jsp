...
<%
    try{
		...
		Connection conn = db.getConnection();
      	...
      	PreparedStatement ps3 = conn.prepareStatement("SELECT * FROM Customer WHERE username = ?");
      	ps3.setString(1,request.getParameter("employeeUsername"));
      	ResultSet rs3 = ps3.executeQuery();
      	
      	if(rs.first() || rs2.first() || rs3.first()){
			out.println("This username already exists.");
			String s = "<form method=\"get\" action=\"./admin.jsp\"><button type=\"submit\">Return</button></form>";
			out.print(s);
      	}else{  
      		ps3 = conn.prepareStatement("INSERT INTO Employee(username, password, first_name, last_name, ssn, access_level) VALUES (?,?,?,?,?,?)");
			ps3.setString(1, request.getParameter("employeeUsername"));
